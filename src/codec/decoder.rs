...
impl BinaryMessageDecoder {
    pub fn decode_message<B>(bytes: &mut B) -> Message
    where
        B: bytes::Buf,
    {
...
impl MessageDecoder for BinaryMessageDecoder {
    fn decode_message<B>(&self, bytes: &mut B) -> Message
    where
        B: bytes::Buf,
    {
    ...
    fn decode_string<B>(&self, bytes: &mut B) -> String
    where
        B: bytes::Buf,
    {
        use bytes::Buf;
        ...
        B: bytes::Buf,
    {
        ...
        match value_type {
            ...
            4 => Value::Float32(self.decode_f32(bytes)),
            5 => Value::Float64(self.decode_f64(bytes)),
            6 => Value::Boolean(self.decode_bool(bytes)),
            7 => Value::Bytes(self.decode_bytes(bytes)),
            8 => Value::List(self.decode_list(bytes)),
            9 => Value::Map(self.decode_map(bytes)),
            10 => Value::Uuid(self.decode_uuid(bytes)),
            _ => panic!("Unsupported value type '{}'", value_type),
        }
    }
    fn decode_i32<B>(&self, bytes: &mut B) -> i32
    where
        B: bytes::Buf,
    {
    ...
    fn decode_list<B>(&self, bytes: &mut B) -> List
    where
        B: bytes::Buf,
    {
    ...
    fn decode_map<B>(&self, bytes: &mut B) -> Map
    where
        B: bytes::Buf,
    {
...
pub trait MessageDecoder {
    ...
    fn decode_bytes<B>(&self, bytes: &mut B) -> Vec<u8>
    where
        B: bytes::Buf;
}
